...
def test_tool_discovery_example_runs() -> None:
    ...
    assert "create_cycle_model" in payload["tool_names"]


def test_session_lifecycle_example_runs() -> None:
    """The session lifecycle example opens and closes a deterministic session."""
    ...
    assert payload["model_name"] == "example-cycle"


@pytest.mark.examples_full
def test_export_snapshot_example_runs() -> None:
    """The export example returns stable output and derivative metadata."""
    completed = _run_example("examples/cpacs/export_snapshot.py")
    assert completed.returncode == 0, completed.stderr

    payload = json.loads(completed.stdout)
    assert payload["fn"] == 5850.0
    assert payload["jacobian_entry"] == [[2.5]]
